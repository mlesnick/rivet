    ...
    , y_grades(bif_data.num_y_grades())
{
    ...
    for (unsigned i = 0; i != mid_generators.size(); i++) {
        ...
        //in grades_vec
        *(mid_generators[i].first->col_inds.begin() +
          std::distance(mid_generators[i].first->grades_vec.begin(),
                        mid_generators[i].first->grades_it++)) = i;
    }
                              ...
                              const SimplexHashMid& mid_ht)
{
    ...
    for (unsigned i = 0; i < high_gens.size(); i++) {
        ...
        if (high_gens[i].first->is_high())
        {
            ...
            for (unsigned k = 0; k < vertices.size(); k++) {
                ...
                while (!found_match) {
                    ...
                    //on R^2?
                    if ((face_node->second->grades_it->x <=
                         high_gens[i].second->x) &&
                        (face_node->second->grades_it->y <=
                         high_gens[i].second->y))
                    {
                        //insert the index in mid_generators corresponding to
                        //this (simplex,grade) pair.
                        high_mx.mat.set(*(face_node->second->col_inds.begin() +
                                          std::distance(face_node->second->grades_vec.begin(),
                                                        face_node->second->grades_it)),i);
                        found_match = true;
                    } else {
        ...
        else
        {
            ...
            high_mx.mat.set(*(high_gens[i].first->col_inds.begin() +
                              std::distance(high_gens[i].first->grades_vec.begin(),
                                            high_gens[i].second) + 1),i);
        
            //add the first index of the relation to the ith column of high_mx
            high_mx.mat.set(*(high_gens[i].first->col_inds.begin() +
                              std::distance(high_gens[i].first->grades_vec.begin(),
                                            high_gens[i].second)),i);
